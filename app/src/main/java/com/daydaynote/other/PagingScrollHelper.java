...
public class PagingScrollHelper {
    ...
    public class MyOnFlingListener extends RecyclerView.OnFlingListener {
        ...
        public boolean onFling(int velocityX, int velocityY) {
            ...
            if (mOrientation == ORIENTATION.VERTICAL) {
                ...
                if (velocityY < 0) {
                    PublicUtil.monthPageId--;
                } else if (velocityY > 0) {
                    PublicUtil.monthPageId++;
                }
                //更具不同的速度判断需要滚动的方向
                //注意，此处有一个技巧，就是当速度为0的时候就滚动会开始的页面，即实现页面复位
                endPoint = PublicUtil.monthPageId * mRecyclerView.getHeight();
                ...
                endPoint = PublicUtil.monthPageId * mRecyclerView.getWidth();

            }
            if (PublicUtil.monthPageId>monthHashmap.size()-1) {
                PublicUtil.monthPageId=monthHashmap.size()-1;
                endPoint = PublicUtil.monthPageId * mRecyclerView.getWidth();
            }
            if (endPoint < 0) {
                endPoint = 0;
                PublicUtil.monthPageId=0;
            }
    ...
    public class MyOnScrollListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            offsetX = PublicUtil.monthPageId*recyclerView.getWidth();
